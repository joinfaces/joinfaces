...
- https://docs.spring.io/spring-boot/docs/{spring-boot-version}/reference/html/howto.html#howto-use-another-web-server[Use Another Web Server]
- https://docs.spring.io/spring-boot/docs/{spring-boot-version}/reference/html/howto.html#howto-configure-log4j-for-logging[Configure Log4j for Logging]
...
https://docs.spring.io/spring-boot/docs/{spring-boot-version}/reference/html/spring-boot-features.html#boot-features-external-config[_Spring Boot_ properties]:
...
* Annotation scanning for none-WARs can be enabled. Make sure to define packages to scan for to decrease the statup time. 

[source,properties]
----
joinfaces.rewrite.scan-classpath=true
joinfaces.rewrite.base-packages=package.with.annotated.components
----
