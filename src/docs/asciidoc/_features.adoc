...
There are seventeen JoinFaces Starters available: six basic starters, two utility starters, one meta starter, eight component starters, one theme starter and one extra starter.
...
NOTE: https://docs.spring.io/spring-webflow/docs/current/reference/html/spring-faces.html#spring-faces-security-taglib[Spring Web Flow] has similar Spring Security JSF Facelet Tag, but tags and functions signatures differ than JoinFaces. More, JoinFaces does not requires additional configuration nor Spring Web Flow.
